...
while [ $i -le 20 ]
do
	n=`cat thr_400.txt | grep "Total throughput" | cut -d':' -f2 | head -$i | tail -1`
	n1=`cat thr_400.txt | grep "Total throughput 1" | cut -d':' -f2 | head -$i | tail -1`
	n2=`cat thr_400.txt | grep "Total throughput 2" | cut -d':' -f2 | head -$i | tail -1`
	cre=`cat thr_400.txt | grep "CRE UEs" | cut -d';' -f2 | head -$i | tail -1`
