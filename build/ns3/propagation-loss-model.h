...
public:
   ...
   */
  void AddShadowing (Ptr<ListPositionAllocator> eNBpositionAllocMacro , uint16_t nUEs);
...
private:
  ...
  virtual void DoAddShadowing (Ptr<ListPositionAllocator> eNBpositionAllocMacro , uint16_t nUEs) = 0;
...
private:
  ...
  virtual double DoCalcRxPower (double txPowerDbm,
                                Ptr<MobilityModel> a,
                                Ptr<MobilityModel> b) const;
  virtual void DoAddShadowing (Ptr<ListPositionAllocator> eNBpositionAllocMacro , uint16_t nUEs);
...
private:
  ...
  double m_exponent;
  double m_freqexponent;
  double m_shadowingstddev;
  double m_shadowingmean;
  double m_referenceDistance;
  double m_refFreq;
  double m_referenceLoss;
  uint8_t m_noOfSectors;

  Ptr<NormalRandomVariable> m_randVariable;
  static std::vector<std::vector<double> > m_shadowing;

  //std::map<Vector3D , uint32_t> ShadowingMapOfeNBs;
