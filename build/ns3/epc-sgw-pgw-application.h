...
public:
  ...
  std::map <uint32_t, Ptr<EpcTft> > GetClassifierEpcTft (Ipv4Address ueAddr);

  void ReactivateS1Bearer (Ipv4Address ueAddr , Ipv4Address enbAddr);
  /** 
  ...
  void MaintainBearerMap (uint64_t imsi, EpsBearer bearer);

  std::vector<EpsBearer> GetBearers (uint64_t imsi);
...
private:
  ...
  public:
    ...
    std::map <uint32_t, Ptr<EpcTft> > GetEpcTft ();

    void RemoveBearer (uint32_t teid);
