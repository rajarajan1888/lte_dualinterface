...
public:
  ...
  /**
     * Add an eNB with aggregated carriers to the EPC
     *
     * \param enbNode the previosuly created eNB node which is to be
     * added to the EPC
     * \param lteEnbNetDeviceContainer the container of the CCs of the eNB node
     */

  void AddEnbCA (Ptr<Node> enbNode, Ptr<NetDevice> lteEnbNetDevice, uint8_t noOfCCs);
  ...
  void AttachUeCA (Ptr<NetDevice> ueLteDevice, uint64_t imsi, Ptr<NetDevice> enbDevice);
  /** 
  ...
  void ReactivateEpsBearer (Ptr<NetDevice> ueLteDevice, uint64_t imsi);
  ...
  void ActivateEpsBearerCA (Ptr<NetDevice> ueLteDevice, uint64_t imsi, Ptr<EpcTft> tft, EpsBearer bearer);
...
private:
  ...
  /**
     * Map storing for each IMSI the corresponding eNB NetDevice CCs
     *
     */
