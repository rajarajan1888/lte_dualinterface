...
LteAmc::CreateCqiFeedbacks (const SpectrumValue& sinr, uint8_t rbgSize)
{
  ...
  //m_shadowing
  static std::ofstream cqivals;
  static std::ofstream specefflogs;
// cqimcs.txt
  static int counter = 0;
  static int logger = 0;
  if(counter==0)
  {
	  cqivals.open("//home//rajarajan//Documents//att_workspace//ltea//cqifeedbacklist.txt");
	  specefflogs.open("//home//rajarajan//Documents//att_workspace//ltea//hetnet//specefflogs.txt",std::ios_base::out|std::ios_base::app);

  }
  cqivals<<"CREATE CQI FEEDBACKS\n";
  counter++;
          ...
          if (sinr_ == 0.0)
          ...
          else
            {
              ...
              */
              ...
              std::cout<<" SINR : "<<sinr<<" Spec Eff : "<<s<<" CQI : "<<cqi_<<std::endl;

              cqivals<<" SINR : "<<sinr<<" Spec Eff : "<<s<<" CQI : "<<cqi_<<"\n";
        ...
        if ((rbId % rbgSize == 0)||((it+1)==sinr.ConstValuesEnd ()))
         {
            ...
            cqivals<< " Time : "<<Simulator::Now().GetMilliSeconds()<<" RBG Size = "<<(uint16_t)rbgSize<<" LTE AMC Module : "<<this<<" RB ID : "<<rbId<<" MCS : "<<(uint16_t)mcs<<" SINR : "<<(*it)<<" CQI : "<<rbgCqi<<" Spectral Efficiency : "<<SpectralEfficiencyForMcs[mcs]<<", TB Size = "<< (GetTbSizeFromMcs (mcs, 48) / 8)<<" Throughput = "<< (GetTbSizeFromMcs (mcs, 48) / 8) / 0.001<<"\n";
            if((uint16_t)rbgSize == 50)
            {
               if(logger<317)
               specefflogs<<SpectralEfficiencyForMcs[mcs]<<","<<GetTbSizeFromMcs (mcs, 48)<<"\n";
               	   logger++;
