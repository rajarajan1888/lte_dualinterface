...
public:
  ...
  virtual void InitialUeMessageCA (uint64_t imsi, uint16_t rnti) = 0;
...
public:
  ...
  virtual void InitialUeMessageCA (uint64_t imsi, uint16_t rnti);
...
{
  std::cout<<"EPC ENB UE MESSAGE (INITIAL)"<<std::endl;
...
template <class C>
void MemberEpcEnbS1SapProvider<C>::InitialUeMessageCA (uint64_t imsi, uint16_t rnti)
{
  m_owner->DoInitialUeMessageCA (imsi, rnti);
}
...
{
	std::cout<<"Data radio Bearer Setup request"<<std::endl;
	// sleep(2);
