...
LteUeRrc::DoForceCampedOnEnb (Ptr<LteEnbNetDevice> enbLteDevice, uint16_t cellId)
{
  ...
  std::cout<<"ForceCamped "<<enbLteDevice<<std::endl;
  ...
  std::cout<<"FORCE CAMPED ON ENB : "<<m_enbRrc<<std::endl;
}
...
{
  NS_LOG_FUNCTION (this);
  std::cout<<"LTE UE RRC Connect"<<std::endl;
...
void
LteUeRrc::DoConnectCA ()
{
	//static int index = 0, rnti;
	NS_LOG_FUNCTION (this);

	//std::cout<<"Problem here? - Index = "<<index<<std::endl;
	//if( index % maxCCs == 0)
	//{
	//	std::cout<<"Gonna invoke the nwe Connection Req CA ???"<<std::endl;
	std::cout<<"DO Connect CA moving on to RecvConnectionRequestCA"<<std::endl;
		m_rnti = m_enbRrc->DoRecvConnectionRequestCA (m_imsi);
	//	rnti = m_rnti;
	//}
    m_cmacSapProvider->ConfigureUe (m_rnti);
	m_cphySapProvider->SetRnti (m_rnti);
...
LteUeRrc::DoRecvConnectionReconfigurationWithMobilityControlInfo (uint16_t targetCellId, uint16_t newRnti)
{
  ...
  static std::ofstream reconc;
  static int counter = 0;
  if(counter==0)
	  reconc.open("//home//rajarajan//Documents//att_workspace//ltea//hetnet//reconcmob.txt");
  counter++;
          ...
          if (!enbDev)
          ...
          else
            {
              if (enbDev->GetCellId () == targetCellId)
                {
            	  std::cout<<enbDev<<" eNBDev Cell ID : "<<enbDev->GetCellId()<<" Target Cell ID : "<<targetCellId<<"\n";
  ...
  // --RAJARAJAN--
  reconc<<"Time : "<<Simulator::Now().GetMilliSeconds()<<" RNTI : "<<m_rnti<<" IMSI : "<<m_imsi<<" Target Cell ID : "<<targetCellId<<"\n";
