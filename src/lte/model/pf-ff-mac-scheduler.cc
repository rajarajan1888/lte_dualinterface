...
PfFfMacScheduler::DoSchedDlRlcBufferReq (const struct FfMacSchedSapProvider::SchedDlRlcBufferReqParameters& params)
{
  ...
  std::ofstream myRlcBuffer;
  myRlcBuffer.open("//home//rajarajan//Documents//att_workspace//ltea//RlcBufferReq.txt");

  if (it == m_rlcBufferReq.end ())
    {
	  myRlcBuffer<<"RLC Trans (IF) Q size parameter = "<<params.m_rlcTransmissionQueueSize<<"\n";
      m_rlcBufferReq.insert (std::pair <LteFlowId_t, FfMacSchedSapProvider::SchedDlRlcBufferReqParameters> (flow, params));
    }
  else
    {
	  myRlcBuffer<<"RLC Trans (ELSE) Q size parameter = "<<params.m_rlcTransmissionQueueSize<<"\n";
      (*it).second = params;
    }
  myRlcBuffer.close();
  return;
...
PfFfMacScheduler::LcActivePerFlow (uint16_t rnti)
{
	...
	static std::ofstream rlcSizes;
	if(counter==0)
		rlcSizes.open("//home//rajarajan//Documents//att_workspace//ltea//hetnet//lcactivecount.txt");
	counter++;
  std::map <LteFlowId_t, FfMacSchedSapProvider::SchedDlRlcBufferReqParameters>::iterator it;
  int lcActive = 0;
  rlcSizes<<this<<" Time : "<<Simulator::Now().GetMilliSeconds()<<" RLC Buffer Size : "<<m_rlcBufferReq.size()<<"\n";
  for (it = m_rlcBufferReq.begin (); it != m_rlcBufferReq.end (); it++)
    {
	  rlcSizes<<" RNTI = "<<(*it).first.m_rnti<<" Transmission Queue size = "<<(*it).second.m_rlcTransmissionQueueSize<<" Retransmission Queue size = "<<(*it).second.m_rlcRetransmissionQueueSize<<" RLC Status PDU size = "<<(*it).second.m_rlcStatusPduSize<<" LC Active = "<<lcActive<<"\n";
	//  std::cout<<"IT FIRST M_RNTI = "<<(*it).first.m_rnti<<" Given RNTI = "<<rnti<<" RLC Trans Queue Size = "<<(*it).second.m_rlcTransmissionQueueSize<<" RLC ReTrans Queue Size = "<<(*it).second.m_rlcRetransmissionQueueSize<<" RLC Status PDU Size = "<<(*it).second.m_rlcStatusPduSize<<std::endl;
  ...
  //std::cout<<"Printing out LcActive value = "<<lcActive<<std::endl;
  return (lcActive);
...
PfFfMacScheduler::DoSchedDlTriggerReq (const struct FfMacSchedSapProvider::SchedDlTriggerReqParameters& params)
{
  ...
  static std::ofstream rgbDetails;
  static int tracker = 0;
  if (tracker == 0)
	  rgbDetails.open("//home//rajarajan//Documents//att_workspace//ltea//rgbDetails.txt");
  tracker++;
  static int ctrTracker = 0;
  static std::filebuf fb;
  if(ctrTracker == 0)
   	 fb.open("//home//rajarajan//Documents//att_workspace//ltea//output_blank.txt",std::ios::out);
  static std::ostream os(&fb);
  uint32_t isBlanked = GetBlankIndicator();

  NS_ASSERT (isBlanked == 0 || isBlanked == 1);

  if (isBlanked == 1)
  {
       	std::cout<<this<<"Blank Indicator is equal to 1 - Macro does not schedule"<<std::endl;
       	os<<this<<" Blank Indicator is equal to 1 (ABS) for "<<(uint32_t)Simulator::Now().GetMilliSeconds()<<"\n";
       	return;
  }

  os<<this<<" Blank Indicator is equal to 0 (non-ABS) for "<<(uint32_t)Simulator::Now().GetMilliSeconds()<<"\n";
  std::cout<<this<<" Blank Indicator is equal to 0 - Macro schedules here"<<std::endl;

  RefreshDlCqiMaps ();

  int rbgSize = GetRbgSize (m_cschedCellConfig.m_dlBandwidth);
  int rbgNum = m_cschedCellConfig.m_dlBandwidth / rbgSize;
  //rgbDetails<<" RBG Size = "<<rbgSize<<" and RBG Num = "<<rbgNum<<"\n";
  ...
  if (m_dlInfoListBuffered.size () > 0)
    {
	  std::cout<<"M_DL INFO LIST BUFFERED SIZE IS GREATER THAN 0"<<std::endl;
      if (params.m_dlInfoList.size () > 0)
        {
    	  std::cout<<"M_DL INFO LIST SIZE IS GREATER THAN 0. RECEIVED DL-HARQ FEEDBACK"<<std::endl;
    ...
    {
	  std::cout<<"M_DL INFO LIST BUFFERED SIZE IS EQUAL TO 0"<<std::endl;
      if (params.m_dlInfoList.size () > 0)
        {
    	  std::cout<<"M_DL INFO LIST SIZE IS GREATER THAN 0. RECEIVED dl info list buffered"<<std::endl;
  ...
  if (m_harqOn == false)
    {
	  ...
	  std::cout<<"HARQ ON = FALSE "<<std::endl;
      m_dlInfoListBuffered.clear ();
    }
  else
	std::cout<<"HARQ ON = TRUE"<<std::endl;

  std::vector <struct DlInfoListElement_s> dlInfoListUntxed;

  static std::ofstream dlInfo;
  static int dlcounter = 0;
  if(dlcounter==0)
  {
	  dlInfo.open("//home//rajarajan//Documents//att_workspace//ltea//hetnet//dlInfoLst.txt",std::ios::out);

  }
  dlcounter++;
  dlInfo<<this<<" DL Info size : "<<m_dlInfoListBuffered.size()<<" and parameters = "<<params.m_dlInfoList.size()<<"\n";


  std::cout<<"M_DL INFO LIST BUFFERED = "<<m_dlInfoListBuffered.size()<<std::endl;
      ...
      if (retx.at (0) || retx.at (1))
        {
          ...
          if (free)
            {
              // use the same RBGs for the retx SRS
              // reserve RBGs
              for (uint8_t j = 0; j < dciRbg.size (); j++)
                {
        ...
        {
          // update HARQ process status SetMacPdu
  ...
  for (int i = 0; i < rbgNum; i++)
    {
      ...
      if (rbgMap.at (i) == false)
        {
          std::map <uint16_t, pfsFlowPerf_t>::iterator it;
          //rgbDetails<<" PRIME CHECK M_FLOWSTATSDL CHECK #0= "<<m_flowStatsDl.size()<<"\n";
          std::map <uint16_t, pfsFlowPerf_t>::iterator itMax = m_flowStatsDl.end ();
          double rcqiMax = 0.0;
          for (it = m_flowStatsDl.begin (); it != m_flowStatsDl.end (); it++)
            {

        	  //rgbDetails<<"RNTI for m_flowStatsDl "<<this<<" = "<<(*it).first<<"\n";
              ...
              if ((itRnti != rntiAllocated.end ())||(!HarqProcessAvailability ((*it).first)))
                {
                  ...
                  if (itRnti != rntiAllocated.end ())
                  {
                //	std::cout<<" RNTI VERY MUCH ALLOCATED - DOES NOT END !!!!"<<std::endl; M_FLOWSTATSDL
            ...
            /*  if(Simulator::Now().GetMilliSeconds()>20)
              {
              rgbDetails<<this<<" A30 Time : "<<Simulator::Now().GetMilliSeconds()<<" CROSS-CHECK CQI : "<<m_a30CqiRxed.size()<<"\n";
              rgbDetails<<this<<" P10 Time : "<<Simulator::Now().GetMilliSeconds()<<" CROSS-CHECK CQI : "<<m_p10CqiRxed.size()<<"\n";
              } */
              std::map <uint16_t,SbMeasResult_s>::iterator itCqi;
              /*if(Simulator::Now().GetMilliSeconds()>20)
              {
              for (itCqi = m_a30CqiRxed.begin() ; itCqi != m_a30CqiRxed.end() ; itCqi++)
            	  rgbDetails<<this<<" CQI cross-check (RNTI) : "<<itCqi->first<<"\n";
              }*/
              itCqi = m_a30CqiRxed.find ((*it).first);
              	  // and Cell ID =
            /*  while(itCqi!=m_a30CqiRxed.end())
              {
            	  rgbDetails<<" CQI cross-check = "<<itCqi->first<<"\n";
            	  itCqi++;
              } */
              ...
              if (itCqi == m_a30CqiRxed.end ())
                ...
                /*  if(Simulator::Now().GetMilliSeconds()>20)
            	  rgbDetails<<" Time = "<<Simulator::Now().GetMilliSeconds()<<" Matching : ITCQI = "<<itCqi->first<<" FLOW STATS DL = "<<it->first<<" CQI1 = "<<(uint16_t)sbCqi.at(0)<<" Number of LC Active Flows = "<<LcActivePerFlow ((*it).first)<<"\n";
				*/
              ...
              if (sbCqi.size () > 1)
                {
            	  std::cout<<"CQI2 - yes greater than 1"<<std::endl;
                  cqi2 = sbCqi.at (1);
                }
        //      std::cout<<" CQI 1 = "<<(uint16_t)cqi1<<" CQI 2 = "<<(uint16_t)cqi2<<std::endl;

              if ((cqi1 > 0)||(cqi2 > 0)) // CQI == 0 means "out of range" (see table 7.2.3-1 of 36.213) RLC Trans Queue Size
                {
                  if (LcActivePerFlow ((*it).first) > 0)
                    {
                      ...
                      NS_LOG_INFO (this << " RNTI " << (*it).first << " MCS " << (uint32_t)mcs << " RBG size = "<<rbgSize<<" achievableRate " << achievableRate << " avgThr " << (*it).second.lastAveragedThroughput << " RCQI " << rcqi);
                      rgbDetails<<this << " Time = "<<Simulator::Now().GetMilliSeconds()<<" Cell ID = "<<m_cschedCellConfig.m_cellId<<" RNTI " << (*it).first << " MCS " << (uint32_t)mcs << " RBG size = "<<rbgSize<<" achievableRate " << achievableRate << " avgThr " << (*it).second.lastAveragedThroughput << " RCQI " << rcqi<<"\n";
                      if (rcqi > rcqiMax)
                        {
                          rcqiMax = rcqi;
                          itMax = it;
          ...
          //std::cout<<" M_FLOWSTATSDL CHECK #1= "<<m_flowStatsDl.size()<<std::endl;

          // Jbro: If the blankIndicator is equal to 1 for this eNB, then we must not schedule any UEs (in this version)
          /*uint32_t isBlanked = GetBlankIndicator();
          if (isBlanked == 1)
          {
        	std::cout<<this<<" Blank Indicator (INSIDE) is equal to 1 - Macro does not schedule"<<std::endl;
        	os<<this<<" Blank Indicator (INSIDE) is equal to 1 (ABS) for "<<(uint32_t)Simulator::Now().GetMilliSeconds()<<"\n";
          	itMax = m_flowStatsDl.end();
          }

          else if (isBlanked == 0)
          {
        	  std::cout<<this<<" Blank Indicator (INSIDE) is equal to 0 - Macro does not schedule"<<std::endl;
        	  os<<this<<" Blank Indicator (INSIDE) is equal to 0 (non-ABS) for "<<(uint32_t)Simulator::Now().GetMilliSeconds()<<"\n";
          }*/

          if (itMax == m_flowStatsDl.end ())
            {
              // no UE available for this RB
        	 // std::cout<<" ITMAX reaches M_FLOWSTATSDL.END() "<<std::endl;
          ...
          else
            {
              ...
              std::cout<<"Mapping Resource Block Groups is true "<<std::endl;
              std::map <uint16_t, std::vector <uint16_t> >::iterator itMap;
              itMap = allocationMap.find ((*itMax).first);
              if (itMap == allocationMap.end ())
                {
            	  std::cout<<" ITMAP == ALLOCATIONMAP.END() : "<<std::endl;
                  // insert new element
                ...
                {
            	  std::cout<<" ITMAP == ALLOCATIONMAP.END() IS NOT TRUE: "<<std::endl;
  ...
  while (itMap != allocationMap.end ())
    {
      ...
      rgb<<" RGB per RNTI "<<(*itMap).first<<" is given by : "<<RgbPerRnti<<"\n";
      std::map <uint16_t,SbMeasResult_s>::iterator itCqi;
      ...
      std::cout<<" M _ BUILD DATA LIST : RET "<<std::endl;
      ret.m_buildDataList.push_back (newEl);
      // update UE stats
      std::map <uint16_t, pfsFlowPerf_t>::iterator it;
  ...
  std::cout<<" M _ NR OF PDCCH OFDM SYMBOLS : RET "<<std::endl;
  ...
  for (itStats = m_flowStatsDl.begin (); itStats != m_flowStatsDl.end (); itStats++)
    {
      ...
      rgb<<"Total bytes transmitted = "<<(*itStats).second.totalBytesTransmitted<<" Throughput = "<<(*itStats).second.lastAveragedThroughput<<"\n";
...
PfFfMacScheduler::DoSchedDlCqiInfoReq (const struct FfMacSchedSapProvider::SchedDlCqiInfoReqParameters& params)
{
  ...
  if(cqicounter==0)
	  confusion.open("//home//rajarajan//Documents//att_workspace//ltea//hetnet//cqitype.txt");
  cqicounter++;
      ...
      if ( params.m_cqiList.at (i).m_cqiType == CqiListElement_s::P10 )
        {
          ...
          confusion<<this<<" Time = "<<Simulator::Now().GetMilliSeconds()<<" RNTI = "<<rnti<<" Reporting format : CQI:P10, Size = "<<m_p10CqiRxed.size()<<"\n";
      ...
      else if ( params.m_cqiList.at (i).m_cqiType == CqiListElement_s::A30 )
        {
          ...
          uint16_t rnti = params.m_cqiList.at (i).m_rnti;
          confusion<<this<<" Time = "<<Simulator::Now().GetMilliSeconds()<<" RNTI = "<<rnti<<" Reporting format : CQI:A30, Size = "<<m_p10CqiRxed.size()<<"\n";
...
PfFfMacScheduler::DoSchedUlTriggerReq (const struct FfMacSchedSapProvider::SchedUlTriggerReqParameters& params)
{
  ...
  static std::ofstream uldcihack;
  static int counter = 0;
  if (counter==0)
	  uldcihack.open("//home//rajarajan//Documents//att_workspace//ltea//hetnet//uldcihack.txt");
  counter++;
  ...
  do
    ...
    /*  if(Simulator::Now().GetMilliSeconds()>200)
      {
    	  std::cout<<" Allocation status : "<<allocated<<" RBs allocated = "<<rbAllocated<<" RBs per flow = "<<rbPerFlow<<" UL bandwidth = "<<m_cschedCellConfig.m_ulBandwidth<<std::endl;
    	  sleep(3);
      } */
      uldcihack<<" Allocation status : "<<allocated<<" RBs allocated = "<<rbAllocated<<" RBs per flow = "<<rbPerFlow<<" UL bandwidth = "<<m_cschedCellConfig.m_ulBandwidth<<"\n";
      ...
      std::cout<<" Time = "<<Simulator::Now().GetMilliSeconds()<<" Allocated status = "<<(!allocated)<<" DCI List = "<<ret.m_dciList.size ()<<std::endl;
          	      //sleep(3);
      if (!allocated)
        {
    ...
    //  sleep(3);
      ...
      if (itCqi == m_ueCqi.end ())
       ...
       //   std::cout<<" (IF) Time = "<<Simulator::Now().GetMilliSeconds()<<" MCS = "<<(uint16_t)uldci.m_mcs<<std::endl;
       //   sleep(3);
        }
      else
        {
       ...
       //   std::cout<<" (ELSE) Time = "<<Simulator::Now().GetMilliSeconds()<<" Min SINR = "<<minSinr<<std::endl;
       //   sleep(3);
          if (minSinr == NO_SINR)
            {
          ...
          std::cout<<" (ELSE) Time = "<<Simulator::Now().GetMilliSeconds()<<" CQI = "<<(uint16_t)cqi<<std::endl;
        //  sleep(3);
          ...
          std::cout<<" (ELSE) Time = "<<Simulator::Now().GetMilliSeconds()<<" MCS = "<<(uint16_t)uldci.m_mcs<<std::endl;
        //  sleep(3);
        }
    ...
    /*  if(Simulator::Now().GetMilliSeconds()>200)
      {
    	  std::cout<<" Time : "<< Simulator::Now().GetMilliSeconds()<<" MCS : "<<(uint16_t)uldci.m_mcs<<" TB Size : "<<uldci.m_tbSize<<std::endl;
    	  sleep(3);
      } */
      uldcihack<<" Time : "<< Simulator::Now().GetMilliSeconds()<<" MCS : "<<(uint16_t)uldci.m_mcs<<" TB Size : "<<uldci.m_tbSize<<"\n";
...
PfFfMacScheduler::RefreshDlCqiMaps (void)
{
  ...
  static std::ofstream refreshdlcqi;
  static int counter = 0;
  if(counter==0)
	  refreshdlcqi.open("//home//rajarajan//Documents//att_workspace//ltea//hetnet//refreshDlCqiMaps.txt");
  counter++;
  while (itP10 != m_p10CqiTimers.end ())
    {
      NS_LOG_INFO (this << " P10-CQI for user " << (*itP10).first << " is " << (uint32_t)(*itP10).second << " thr " << (uint32_t)m_cqiTimersThreshold);
      refreshdlcqi<<this<<"Time : "<<Simulator::Now().GetMilliSeconds()<<" P10-CQI for user " << (*itP10).first << " is " << (uint32_t)(*itP10).second << " thr " << (uint32_t)m_cqiTimersThreshold<<"\n";
      if ((*itP10).second == 0)
        {
          // delete correspondent entries
    	  refreshdlcqi<<this<<"Time : "<<Simulator::Now().GetMilliSeconds()<<" Deleting corresponding entries"<<"\n";
  ...
  while (itA30 != m_a30CqiTimers.end ())
    {
      ...
      refreshdlcqi<<this<<"Time : "<<Simulator::Now().GetMilliSeconds()<<" A30-CQI for user " << (*itA30).first << " is " << (uint32_t)(*itA30).second << " thr " << (uint32_t)m_cqiTimersThreshold<<"\n";
      if ((*itA30).second == 0)
        {
          // delete correspondent entries
    	  refreshdlcqi<<this<<"Time : "<<Simulator::Now().GetMilliSeconds()<<" Deleting corresponding entries"<<"\n";
