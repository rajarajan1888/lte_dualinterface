...
// RAJARAJAN
void
EpcUeNas::ConnectCA (Ptr<NetDevice> enbDevice)
{
 ...
 /* if(index % maxCCs == 0)
  {
	  std::cout<<"Connecting to the new UE ????? "<<std::endl;
	  m_asSapProvider->ConnectCA (maxCCs);
  }*/
  std::cout<<"Connect CA moving on to SAP Provider Connect CA"<<std::endl;
  m_asSapProvider->ConnectCA ();
  if (m_epcHelper)
    {
      m_epcHelper->AttachUeCA (m_device, m_imsi, enbDevice);
      // also activate default EPS bearer
      ActivateEpsBearerCA (EpsBearer (EpsBearer::NGBR_VIDEO_TCP_DEFAULT), EpcTft::Default ());
    }
...
void 
EpcUeNas::ActivateEpsBearerCA (EpsBearer bearer, Ptr<EpcTft> tft)
{
  NS_ASSERT_MSG (m_bidCounter < 11, "cannot have more than 11 EPS bearers");
  uint8_t bid = ++m_bidCounter;
  m_epcHelper->ActivateEpsBearerCA (m_device, m_imsi, tft, bearer);
...
EpcUeNas::Connect (Ptr<NetDevice> enbDevice)
{
  ...
  std::cout<<"EPC UE NAS Connect "<<std::endl;
  ...
  std::cout<<"EPS Traffic?"<<std::endl;
  if (m_epcHelper)
    {
	  std::cout<<"EPS traffic - YIPPEEEY"<<std::endl;
      m_epcHelper->AttachUe (m_device, m_imsi, enbDevice);
      // also activate default EPS bearer
      if(bearerActivated==false)
      {
    	  ActivateEpsBearer (EpsBearer (EpsBearer::NGBR_VIDEO_TCP_DEFAULT), EpcTft::Default ());
    	  bearerActivated = true;
      }
      else
    	  m_epcHelper->ReactivateEpsBearer (m_device, m_imsi);
    }
}
...
{
  NS_LOG_FUNCTION (this);
  //std::cout<<"EPC UE NAS - DoRecvData"<<std::endl;
  //sleep(3);
