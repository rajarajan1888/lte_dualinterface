    ...
    m_expectedSeqNumber (0)
{
  ...
  counter1++ ;
  std::ofstream myBytes;
  myBytes.open("RLC_UM_Constructor.txt");
  myBytes<<"Tx Buffer Size = "<<m_txBufferSize<<" Counter = "<<counter1<<"\n";
...
LteRlcUm::DoTransmitPdcpPdu (Ptr<Packet> p)
{
  ...
  static std::ofstream myBytes;

  if(counter==0)
	  myBytes.open("//home//rajarajan//Documents//att_workspace//ltea//hetnet//Pdcp.txt");
  counter++;

  myBytes<<this<<" Time : "<<Simulator::Now().GetMilliSeconds()<<" Do PDCP : "<<p->GetSize()<<", Max Tx Buffer Size = "<<m_maxTxBufferSize<<" Tx Buffer Size = "<<m_txBufferSize<<"Counter = "<<counter<<"\n";
  ...
  if (m_txBufferSize + p->GetSize () <= m_maxTxBufferSize)
    {
      ...
      myBytes<<"CONDITION TRUE : Do PDCP : "<<p->GetSize()<<", Max Tx Buffer Size = "<<m_maxTxBufferSize<<" Tx Buffer Size = "<<m_txBufferSize<<"Counter = "<<counter<<"\n";
    }
  else
    {
...
LteRlcUm::DoNotifyTxOpportunity (uint32_t bytes, uint8_t layer, uint8_t harqId)
{
 ...
 // static int counter = 0;
  ...
  static int counter = 0;
  if(counter == 0)
  myBytes.open("//home//rajarajan//Documents//att_workspace//ltea//hetnet//TxBytes.txt");
  counter++;
  std::cout<<"Do Notify Tx Opportunity : Time = "<<Simulator::Now().GetMilliSeconds()<<" Bytes = "<<bytes<<" Layer = "<<layer<<" HARQ ID = "<<harqId<<std::endl;
  //sleep(2);
...
{
  NS_LOG_FUNCTION (this << m_rnti << (uint32_t) m_lcid << p->GetSize ());
  static std::ofstream lterlcum;
  static int counter = 0;
  if(counter==0)
	  lterlcum.open("//home//rajarajan//Documents//att_workspace//ltea//hetnet//lterlcum.txt");
  counter++;
  // Receiver timestamp
  RlcTag rlcTag;
  Time delay;
  ...
  //std::cout<<" RNTI = "<<m_rnti<<" LCID = "<<m_lcid<<" Packet size = "<<p->GetSize()<<" Time = "<<Simulator::Now().GetMilliSeconds()<<std::endl;
  //sleep(2);
  lterlcum<<" RNTI = "<<m_rnti<<" LCID = "<<m_lcid<<" Packet size = "<<p->GetSize()<<" Time = "<<Simulator::Now().GetMilliSeconds()<<"\n";
...
LteRlcUm::DoReportBufferStatus (void)
{
  ...
  static std::ofstream myFileRlc;
  static int counter = 0;
  if(counter==0)
      myFileRlc.open("//home//rajarajan//Documents//att_workspace//ltea//hetnet//LTE_RLC_UM.txt");
  counter++;
  myFileRlc<<this<<" Time = "<<Simulator::Now().GetMilliSeconds()<<" Tx Buffer size = "<<m_txBuffer.size()<<"\n";

  if (! m_txBuffer.empty ())
    {
	  myFileRlc << "Buffer not empty \n";
      RlcTag holTimeTag;
      ...
      myFileRlc<<" RLC UM Queue size : "<<queueSize<<"\n";
    }

  else
	  myFileRlc << "Buffer empty at the moment\n";
