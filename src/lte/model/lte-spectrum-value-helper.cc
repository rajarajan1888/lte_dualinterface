  ...
  for (uint16_t i = 0; i < NUM_EUTRA_BANDS; ++i)
    {
      if ((g_eutraChannelNumbers[i].rangeNdl1 <= nDl)
          && (g_eutraChannelNumbers[i].rangeNdl2 >= nDl))
        {
          ...
          std::ofstream dlFreq;
          dlFreq.open("//home//rajarajan//Documents//att_workspace//ltea//DlFreq.txt");
          dlFreq<<"CB Freq : "<<1.0e6 * (g_eutraChannelNumbers[i].fDlLow + 0.1 * (nDl - g_eutraChannelNumbers[i].nOffsDl))<<"\n";
          dlFreq.close();
  ...
  for (uint16_t i = 0; i < NUM_EUTRA_BANDS; ++i)
    {
      if ((g_eutraChannelNumbers[i].rangeNul1 <= nUl)
          && (g_eutraChannelNumbers[i].rangeNul2 >= nUl))
        {
          ...
          std::ofstream ulFreq;
                   ulFreq.open("//home//rajarajan//Documents//att_workspace//ltea//UlFreq.txt");
                   ulFreq<<"CB Freq : "<<1.0e6 * (g_eutraChannelNumbers[i].fUlLow + 0.1 * (nUl - g_eutraChannelNumbers[i].nOffsUl))<<"\n";
                   ulFreq.close();
...
LteSpectrumValueHelper::CreateNoisePowerSpectralDensity (double noiseFigureDb, Ptr<SpectrumModel> spectrumModel)
{
  ...
  //std::cout<<"Noise PSD = "<<noisePowerSpectralDensity<<std::endl;-174
  //sleep(5);
