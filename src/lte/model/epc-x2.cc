...
EpcX2::AddX2Interface (uint16_t enb1CellId, Ptr<Socket> enb1X2cSocket, uint16_t enb2CellId, Ptr<Socket> enb2X2cSocket)
{
  ...
  static std::filebuf fb;
   ...
   if(counter == 0)
     fb.open("//home//rajarajan//Documents//att_workspace//ltea//hetnet//interfaces.txt",std::ios::out);
   counter++;
   static std::ostream os(&fb);
   os<<this<<" Interface between Cell 1 "<<enb1CellId<<" and Cell 2 "<<enb2CellId<<"\n";
   std::cout<<this<<" EPC X2 : Cell 1 = "<<enb1CellId<<" Cell 2 = "<<enb2CellId<<std::endl;
      ...
      if (messageType == EpcX2Header::INITIATING_MESSAGE)
        {
          ...
          params.oldEnbUeX2apId = x2HoReqHeader.GetOldRnti ();
          //params.ueRrc 			= DynamicCast<LteUeRrc>(x2HoReqHeader.GetUeRrc ());

          std::cout<<"RECV HANDOVER : Time : "<<Simulator::Now().GetMilliSeconds()<<" Old RNTI = "<<params.oldEnbUeX2apId<<" Source Cell ID = "<<params.sourceCellId<<" Target Cell ID = "<<params.targetCellId<<std::endl;
     //     sleep(2); and Cell ID =
      ...
      else // messageType == SUCCESSFUL_OUTCOME
        {
          ...
          params.oldEnbUeX2apId = x2HoReqAckHeader.GetOldRnti ();
          //params.ueRrc = DynamicCast<LteUeRrc>(x2HoReqAckHeader.GetUeRrc ());
...
EpcX2::DoSendHandoverRequest (EpcX2SapProvider::HandoverRequestParams params)
{
  ...
  std::cout<<"DO HANDOVER REQ. Time = "<<Simulator::Now().GetMilliSeconds()<<" IMSI = "<<params.oldEnbUeX2apId<<" Source Cell ID = "<<params.sourceCellId<<" Target Cell ID = "<<params.targetCellId<<std::endl;
  //sleep(2);
  ...
  x2HoReqHeader.SetOldRnti(params.oldEnbUeX2apId);

  //x2HoReqHeader.SetUeRrc(params.ueRrc);
  ...
  std::cout<<this<<" BEFORE SENDING TO : Time = "<<Simulator::Now().GetMilliSeconds()<<" Original RNTI = "<<params.oldEnbUeX2apId<<" Source Cell ID = "<<params.sourceCellId<<" Target Cell ID = "<<params.targetCellId<<std::endl;
 // sleep(2);
  // Send the X2 message through the socket

  int x = sourceSocket->SendTo (packet, 0, InetSocketAddress (targetIpAddr, m_x2cUdpPort));

  std::cout<<"X return value : "<<x<<std::endl;
 // sleep(2); : public Socket
