...
EpcSgwPgwApplication::UeInfo::GetEpcTft ()
{
	return m_tftClassifier.GetEpcTft();
}

void
EpcSgwPgwApplication::UeInfo::RemoveBearer (uint32_t teid)
{

  m_tftClassifier.Delete(teid);
}
...
std::vector<EpsBearer>
EpcSgwPgwApplication::GetBearers (uint64_t imsi)
{
	std::vector<EpsBearer> bearers;
	std::pair <std::multimap<uint64_t, EpsBearer>::iterator, std::multimap<uint64_t, EpsBearer>::iterator > ret;
	ret = EpsBearerMultiMap.equal_range(imsi);
	for(std::multimap<uint64_t, EpsBearer>::iterator it = ret.first ; it != ret.second; it++)
		bearers.push_back(it->second);
	return bearers;
}

void
EpcSgwPgwApplication::MaintainBearerMap (uint64_t imsi , EpsBearer bearer)
{
	EpsBearerMultiMap.insert(std::pair<uint64_t, EpsBearer>(imsi,bearer));
}
...
std::map <uint32_t, Ptr<EpcTft> >
EpcSgwPgwApplication::GetClassifierEpcTft (Ipv4Address ueAddr)
{
	std::map<Ipv4Address, UeInfo>::iterator it = m_ueInfoMap.find (ueAddr);
	NS_ASSERT_MSG(it != m_ueInfoMap.end (), "UE not found");
    return it->second.GetEpcTft();
}

void
EpcSgwPgwApplication::ReactivateS1Bearer (Ipv4Address ueAddr , Ipv4Address enbAddr)
{
	std::map<Ipv4Address, UeInfo>::iterator it = m_ueInfoMap.find(ueAddr);
	if ( it == m_ueInfoMap.end())
	 it->second.SetEnbAddr (enbAddr);
