...
LteSpectrumPhy::SetNoisePowerSpectralDensity (Ptr<const SpectrumValue> noisePsd)
{
  ...
  std::cout<<"Noise PSD = "<<noisePsd<<std::endl;
...
LteSpectrumPhy::SetLtePhyDlHarqFeedbackCallback (LtePhyDlHarqFeedbackCallback c)
{
  NS_LOG_FUNCTION (this);
  std::ofstream myFile;
        myFile.open("//home//rajarajan//Documents//att_workspace//ltea//SetLtePhyDlHarqFeedbackCallback.txt");
...
LteSpectrumPhy::StartTxDataFrame (Ptr<PacketBurst> pb, std::list<Ptr<LteControlMessage> > ctrlMsgList, Time duration)
{
  ...
  static std::ofstream myTxDataFrame;
  static int counter = 0;
  if(counter==0)
	  myTxDataFrame.open("//home//rajarajan//Documents//att_workspace//ltea//TxDataFrame_IDLE.txt");
  counter++;
  ...
  switch (m_state)
  {
    ...
    case IDLE:
    {
      ...
      */
      ...
      //myTxDataFrame<<"Start TX Data Frame IDLE: "<<pb<<" Time : "<<Simulator::Now().GetMilliSeconds()<<"Channel : "<<m_channel<<"\n";
      ...
      if(m_direction=='D')
    	  m_channel->SetDirection('D');
      else if(m_direction=='U')
    	  m_channel->SetDirection('U');

      if(txParams->GetPacketBurst())
      {
    	  myTxDataFrame<<this<<" Start TX Data Frame IDLE: "<<txParams->GetPacketBurst ()<<" Tx Params PHY = "<<txParams->txPhy<<" Time : "<<Simulator::Now().GetMilliSeconds()<<"Channel : "<<m_channel<<" CTRL Msg : "<<ctrlMsgList.size()<<" No. of packets = "<<txParams->packetBurst->GetNPackets()<<" Size : "<<txParams->packetBurst->GetSize()<<"\n";
      }
...
void
LteSpectrumPhy::SetDirection (uint8_t direction)
{
...
LteSpectrumPhy::StartTxDlCtrlFrame (std::list<Ptr<LteControlMessage> > ctrlMsgList)
{
  ...
  static std::ofstream myTxCtrlFrame;
  static int counter = 0;
  if(counter==0)
 	  myTxCtrlFrame.open("//home//rajarajan//Documents//att_workspace//ltea//hetnet//txCtrl.txt");
  counter++;
...
//   m_phyTxStartTrace (pb);
  ...
  switch (m_state)
  {
    ...
    {
      /*
      m_txPsd must be setted by the device, according to GetControlMsg
      (i) the available subchannel for transmission
      (ii) the power transmission
      */
      NS_ASSERT (m_txPsd);
      ...
      txParams->cellId = m_cellId;
      txParams->ctrlMsgList = ctrlMsgList;
      myTxCtrlFrame<<" Time : "<<Simulator::Now().GetMilliSeconds()<<" CTRL Msg : "<<ctrlMsgList.size() <<" Cell ID : "<<txParams->cellId<<"\n";
      m_channel->AddType('C');
...
LteSpectrumPhy::StartRx (Ptr<SpectrumSignalParameters> spectrumRxParams)
{
  ...
  static std::ofstream myInterf;
  static int counter = 0;
  if(counter==0)
	  myInterf.open("//home//rajarajan//Documents//att_workspace//ltea//data_ctrl.txt");
  counter++;
  ...
  // understood by this device - in this case, an LTE signal.  FIRST SIGNAL
  ...
  if(spectrumRxParams->packetBurst)
  {
  lteDataRxParams->packetBurst=spectrumRxParams->packetBurst;

  }
  //Ptr<LteSpectrumSignalParametersDataFrame> lteDataRxParams = (Ptr<LteSpectrumSignalParametersDataFrame> (spectrumRxParams);
  if (lteDataRxParams != 0)
    {
	  if(spectrumRxParams->packetBurst)
	  myInterf<<this<<"Time : "<<Simulator::Now().GetMilliSeconds()<<" Data : "<<m_interferenceData<<" packet burst packets : "<< spectrumRxParams->packetBurst->GetNPackets()<<" Size : "<<spectrumRxParams->packetBurst->GetSize()<<"\n";
	  else
	  myInterf<<this<<"Time : "<<Simulator::Now().GetMilliSeconds()<<" Data : "<<m_interferenceData<<" and no packets, CTRL size = "<<lteDataRxParams->ctrlMsgList.size()<<"\n";
	  m_interferenceData->AddType ('d');
      m_interferenceData->AddSignal (rxPsd, duration);
  //    StartRxData (lteDataRxParams); GetControlMessages
      StartRxData (lteDataRxParams);
    }
  else
    {
	  if(spectrumRxParams->packetBurst)
	  	  myInterf<<this<<"Time : "<<Simulator::Now().GetMilliSeconds()<<" CTRL : "<<m_interferenceData<<" packet burst packets : "<< spectrumRxParams->packetBurst->GetNPackets()<<" Size : "<<spectrumRxParams->packetBurst->GetSize()<<"\n";
	  	  else
	  	  myInterf<<this<<"Time : "<<Simulator::Now().GetMilliSeconds()<<" CTRL : "<<m_interferenceData<<" and no packets\n";
...
LteSpectrumPhy::StartRxData (Ptr<LteSpectrumSignalParametersDataFrame> params)
{
  ...
  static std::ofstream myFile6;
          	static int datactr = 0;
          	if(datactr == 0)
          	myFile6.open("//home//rajarajan//Documents//att_workspace//ltea//DataReceive.txt");
          	datactr++;
          	if(params->packetBurst)

          	myFile6 <<this<< " Time : "<< Simulator::Now().GetMilliSeconds() <<" Data receive : Just a check : PARAMS->cellID = "<<params->cellId<<" and m_cellID = "<<m_cellId<<" Packet burst packets = "<< params->packetBurst->GetNPackets()<<" Packet size : "<<params->packetBurst->GetSize()<<"\n";
          	else
          	myFile6 <<this<< " Time : "<<Simulator::Now().GetMilliSeconds() << "Data receive : Just a check : PARAMS->cellID = "<<params->cellId<<" and m_cellID = "<<m_cellId<<" CTRL msg = "<<params->ctrlMsgList.size()<<"\n";
  switch (m_state)
    {
      case TX:
      ...
      case RX_DATA:
        ...
        // simultaneously (e.g., at the eNB). 888-
        {
          ...
          //  LteSpectrumSignalParametersDataFrame
            ...
            if(params->cellId  == m_cellId)
            {
              ...
              if ((m_rxPacketBurstList.empty ())&&(m_rxControlMessageList.empty ()))
                {
                  ...
                  myFile6<<this<<" scheduling EndRx with delay " << params->duration.GetSeconds () << "s\n";
              ...
              if (params->packetBurst)
                ...
                myFile6<<" Checking Control Message List : "<<m_rxControlMessageList.size()<<"\n";
...
LteSpectrumPhy::StartRxCtrl (Ptr<SpectrumSignalParameters> params)
{
  ...
  switch (m_state)
  {
    ...
    case RX_CTRL:
      // the behavior is similar when
      // we're IDLE or RX because we can receive more signals
      // simultaneously (e.g., at the eNB). writeControlMsg.txt
      {
        // To check if we're synchronized to this signal, we check
        // for the CellId which is reported in the
        // LteSpectrumSignalParametersDlCtrlFrame
        ...
        if (cellId  == m_cellId)
        {
          ...
          /*if(m_device->GetObject<LteUeNetDevice>())
          std::cout<<" Time : "<<Simulator::Now().GetMilliSeconds()<<" IMSI = "<<m_device->GetObject<LteUeNetDevice>()->GetImsi()<<" and Cell ID = "<<m_cellId<<std::endl;*/
        /*  if(Simulator::Now().GetMilliSeconds()>50)
        	  sleep(2);*/ // CreateSRSReport
          if (m_state == IDLE)
          {
            // first transmission, i.e., we're IDLE and we
            // start RX
...
{
  static std::ofstream sinrPerceived;
  static int counter = 0;
  if(counter==0)
	  sinrPerceived.open("//home//rajarajan//Documents//att_workspace//ltea//hetnet//sinrPxd.txt");
  counter++;
  NS_LOG_FUNCTION (this << sinr);
  m_sinrPerceived = sinr;
  sinrPerceived<<this<<m_sinrPerceived<<"\n";
...
{
	static int counter = 0;
	static std::ofstream tbsizetracker;
	if(counter==0)
		tbsizetracker.open("//home//rajarajan//Documents//att_workspace//ltea//hetnet//tbsizetracker.txt");
	counter++;
	tbsizetracker<<" Time : "<<Simulator::Now().GetMilliSeconds()<<" rnti: " << rnti << " NDI " << (uint16_t)ndi << " size " << size << " mcs " << (uint16_t)mcs << " layer " << (uint16_t)layer << "\n";
...
LteSpectrumPhy::EndRxData ()
{
  ...
  static std::ofstream rxpkts;
  static int rxcounter = 0;

  if(rxcounter==0)
  	  rxpkts.open("//home//rajarajan//Documents//att_workspace//ltea//hetnet//DlPackets.txt");
  rxcounter++;
  rxpkts<<this<<" Time : "<<Simulator::Now().GetMilliSeconds ()<<" Interference : "<<m_interferenceData<<"Tracking SINR perceived\n";
  rxpkts<<m_sinrPerceived<<"\n";
  ...
  {
	  rxpkts<<" Iterator value : "<<(*it)<<"\n";
	  sum = sum + (*it);
	  count++;
  }
  double avgSinr = sum/(double)count;

  rxpkts<<this<<" Burst size : "<<m_rxPacketBurstList.size ()<<" SINR : "<<avgSinr<<" TBs : "<<m_expectedTbs.size ()<<" Rx Packet burst = "<<m_rxPacketBurstList.size ()<<"\n";
  std::cout<<" Burst size : "<<m_rxPacketBurstList.size ()<<" SINR : "<<avgSinr<<" TBs : "<<m_expectedTbs.size ()<<" Rx Packet burst = "<<m_rxPacketBurstList.size ()<<std::endl;
  //sleep(3);
      ...
      if ((m_dataErrorModelEnabled)&&(m_rxPacketBurstList.size ()>0)) // avoid to check for errors when there is no actual data transmitted
        {
          ...
          rxpkts<<"RNTI " << (*itTb).first.m_rnti << " size " << (*itTb).second.size << " mcs " << (uint32_t)(*itTb).second.mcs << " bitmap " << (*itTb).second.rbBitmap.size () << " layer " << (uint16_t)(*itTb).first.m_layer << " TBLER " << tbStats.tbler << " corrupted " << (*itTb).second.corrupt<<" received packet burst = "<< m_rxPacketBurstList.size()<<" \n";
       }
            ...
            if (itTb!=m_expectedTbs.end ())
              {
   	              ...
   	              if (!(*itTb).second.corrupt)
                  {
                    ...
                    if (!m_ltePhyRxDataEndOkCallback.IsNull ())
                      {
                    // 	std::cout<<" Rx Packet burst list = "<<m_rxPacketBurstList.size()<<std::endl;
                    //	sleep(3); // IPv4 Static Routing
  ...
  static int counter = 0;
  if(counter==0)
  	  myFile1.open("//home//rajarajan//Documents//att_workspace//ltea//LtePhyDlHarqCallBack.txt");
  counter++;
  myFile1<<" Harq DL Info Map begins : "<<harqDlInfoMap.size()<<"\n";
  for (itHarq = harqDlInfoMap.begin (); itHarq != harqDlInfoMap.end (); itHarq++)
    {
      ...
      if (!m_ltePhyDlHarqFeedbackCallback.IsNull ())
        {
    	//  std::ofstream myFile;
    	        myFile1<<" Not Null "<<((*itHarq).second.m_rnti)<<"\n";
          m_ltePhyDlHarqFeedbackCallback ((*itHarq).second);
        }
    }
  // forward control messages of this frame to LtePhy    m_ltePhyUlHarqFeedbackCallback
  if (!m_rxControlMessageList.empty ())
    {
	  /*myFile1.open("//home//rajarajan//Documents//att_workspace//ltea//CtrlMsgList.txt");
	  myFile1 << "Control \n";
	  myFile1.close();*/

        myFile1 << "Control Msg list is NOT empty \n";

      if (!m_ltePhyRxCtrlEndOkCallback.IsNull ())
        {
    	  /*myFile1.open("//home//rajarajan//Documents//att_workspace//ltea//LteCqiList.txt");*/
// Send Data Packet transmission
    	rxpkts << "Time : "<<Simulator::Now().GetMilliSeconds ()<<" EndRxData\n";
...
LteSpectrumPhy::EndRxDlCtrl ()
{
  NS_LOG_FUNCTION (this);
  NS_LOG_LOGIC (this << " state: " << m_state);
  static std::ofstream rxCtrl;
  static int counter = 0;
  if(counter==0)
	  rxCtrl.open("//home//rajarajan//Documents//att_workspace//ltea//endRxDlCtrl.txt");
  rxCtrl<<"Hi. I am "<<this<<"\n";
  counter++;
...
//   m_sinrPerceived *= m_txModeGain.at (m_transmissionMode); ctrlMsgList SendControlChannels.txt
  bool error = false;
      ...
      if (!m_ltePhyRxCtrlEndOkCallback.IsNull ())
        {
    	  ...
    	  rxCtrl << "Time : "<<Simulator::Now().GetMilliSeconds ()<<" EndRxCtrl\n";
