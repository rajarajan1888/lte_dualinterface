...
LteCtrlSinrChunkProcessor::End ()
{
  ...
  if (m_totDuration.GetSeconds () > 0)
    {
//	  std::cout<<"WILD CHECK : M_SUMSINR = "<<(*m_sumSinr)<<std::endl;
...
{
  NS_LOG_FUNCTION (this);
  static std::ofstream datachunk;
  static int counter = 0;
  if(counter==0)
	  datachunk.open("//home//rajarajan//Documents//att_workspace//ltea//chunkdata.txt");
  counter++;
  datachunk<<"Entering Data Sinr Chunk Processor - End\n";
  if (m_totDuration.GetSeconds () > 0)
  {
	datachunk<<"Updating SINR\n";
    m_spectrumPhy->UpdateSinrPerceived ((*m_sumSinr) / m_totDuration.GetSeconds ());
    if (m_phy)
      {
    	datachunk<<"Updating Data CQI report\n";
