...
bool LteEnbNetDevice::isCA=0;
uint8_t LteEnbNetDevice::maxCCs = 0;
...
void
LteEnbNetDevice::SetCellBias (uint8_t cellBias)
{
	m_cellBias = cellBias;
}

uint8_t
LteEnbNetDevice::GetCellBias (void) const
{
	return m_cellBias;
}
...
{
  NS_LOG_FUNCTION (this);
  std::cout<<"ENB PHY:"<<m_phy<<std::endl;
  return m_phy;
}
...
{
	  std::cout<<"ENB PHY ADDRESS (LTE ENB NET DEVICE) : "<<m_phy<<std::endl;
	  std::cout<<"Next step from dev creation ? - address of m_rrc = "<<m_rrc<<std::endl;
...
void
LteEnbNetDevice::StartCA (uint8_t noOfCCs)
{
	isCA = 1;
	maxCCs = noOfCCs;
	NS_ABORT_MSG_IF (m_cellIdCounter == 65535, "max num eNBs exceeded");
...
LteEnbNetDevice::Send (Ptr<Packet> packet, const Address& dest, uint16_t protocolNumber)
{
  ...
  std::cout<<"Happening here at eNB end ? "<<std::endl;
  //  sleep(2);

  std::ofstream myPacket;
  myPacket.open("//home//rajarajan//Documents//att_workspace//ltea//packetsend.txt");
  myPacket<<"I'm being called\n";
  myPacket.close();

  NS_ASSERT_MSG (protocolNumber == Ipv4L3Protocol::PROT_NUMBER, "unsupported protocol " << protocolNumber << ", only IPv4 is supported");
  return m_rrc->Send (packet); // ->Send (
}

void
LteEnbNetDevice::UpdateConfigCA (uint8_t noOfCCs)
{
  NS_LOG_FUNCTION (this);

  m_rrc->ConfigureCellCA (m_ulBandwidth, m_dlBandwidth, m_ulEarfcn, m_dlEarfcn, m_cellId, noOfCCs);
...
Ptr<FfMacScheduler>
LteEnbNetDevice::GetScheduler(void) const
{
	NS_LOG_FUNCTION (this);
	return m_scheduler;
}
