...
PhyStatsCalculator::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::PhyStatsCalculator")
    ...
    .AddAttribute ("DlRsrpSinrFilename",
                   ...
                   StringValue ("DlRsrpSinrStatsHetNet.txt"),
    ...
    .AddAttribute ("UlSinrFilename",
                   ...
                   StringValue ("UlSinrStatsHetNet.txt"),
    ...
    .AddAttribute ("UlInterferenceFilename",
                   ...
                   StringValue ("UlInterferenceStatsHetNet.txt"),
...
void
PhyStatsCalculator::ReportCurrentCellTbs (uint16_t cellId , uint16_t rnti , uint16_t tbsSize)
{
	 	 static std::filebuf fb;
		 static int counter = 0;
		 if(counter == 0)
		  fb.open("//home//rajarajan//Documents//att_workspace//ltea//hetnet//tbsStats.txt",std::ios::out);
		 counter++;
		 static std::ostream os(&fb);
		 // UL DCI
	//	 std::cout<<" CELL ID: "<<cellId<<" IMSI: "<<imsi<<" RSRP: "<<rsrp<<" and in dB, it is "<<10*log10(rsrp*180000*1000)<<std::endl;
		 os<<" CELL ID: "<<cellId<<" RNTI: "<<rnti<<" TBS Size "<<tbsSize<<"\n";
}

void
PhyStatsCalculator::ReportAllCellRsrp (uint16_t cellId , uint64_t imsi , double rsrp)
{
	 static std::filebuf fb;
		 static int counter = 0;
		 if(counter == 0)
		  fb.open("//home//rajarajan//Documents//att_workspace//ltea//hetnet//rsrpAllStats.txt",std::ios::out);
		 counter++;
		 static std::ostream os(&fb);
		 // UL DCI
	//	 std::cout<<" CELL ID: "<<cellId<<" IMSI: "<<imsi<<" RSRP: "<<rsrp<<" and in dB, it is "<<10*log10(rsrp*180000*1000)<<std::endl;
	 os<<" CELL ID: "<<cellId<<" IMSI: "<<imsi<<" RSRP: "<<rsrp<<" and in dB, it is "<<10*log10(rsrp*180000*1000)<<"\n";
}
                                               ...
                                               double rsrp, double sinr)
{
  ...
  std::cout<<"I AM GONNA WRITE TO THE FILE : "<<GetCurrentCellRsrpSinrFilename ().c_str ()<<std::endl;
  ...
  if ( m_RsrpSinrFirstWrite == true )
      ...
      if (!outFile.is_open ())
        {
          ...
          std::cout<<"cannot open file here: I'm sorry here OH OH OH - PhyStats"<<std::endl;
          return;
        }

      std::cout<<"I AM GONNA WRITE TO THE FILE AND YES THIS FILE IS OPEN: "<<GetCurrentCellRsrpSinrFilename ().c_str ()<<std::endl;
  ...
  else
    {
      ...
      if (!outFile.is_open ())
        {
          NS_LOG_ERROR ("Can't open file " << GetCurrentCellRsrpSinrFilename ().c_str ());
          std::cout<<"cannot open file here: I'm sorry here OH OH OH - PhyStats Append"<<std::endl;
          return;
        }
    }
...
PhyStatsCalculator::ReportUeSinr (uint16_t cellId, uint64_t imsi, uint16_t rnti,  double sinrLinear)
{
  ...
  std::cout<<"Cell ID : "<<cellId<<" IMSI : "<<imsi<<" RNTI : "<<rnti<<std::endl;
  outFile << Simulator::Now ().GetNanoSeconds () / (double) 1e9 << "\t";
  outFile << cellId << "\t";
  outFile << imsi << "\t";
  outFile << rnti << "\t";
