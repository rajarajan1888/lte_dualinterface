...
std::vector<std::vector<double> > rsrpVector;
std::vector<std::vector<double> > allRsrpVector;
std::vector<std::vector<double> > sinrVector;
std::vector<std::vector<double> > tbsVector;

std::vector<std::vector<double> > LteHelper::rsrpVtr = rsrpVector;
std::vector<std::vector<double> > LteHelper::rsrpAllVtr = allRsrpVector;
std::vector<std::vector<double> > LteHelper::sinrVtr = sinrVector;
std::vector<std::vector<double> > LteHelper::tbsVtr = tbsVector;

uint32_t noOfUEs , noOfCells;
...
LteHelper::LteHelper (uint32_t nUEs, uint32_t nCells)
{
	std::cout<<"CONSTRUCTOR SUCCESSFULLY INVOKED"<<std::endl;
	NS_LOG_FUNCTION (this);
	noOfUEs = nUEs;
	noOfCells = nCells;
	m_enbNetDeviceFactory.SetTypeId (LteEnbNetDevice::GetTypeId ());
	...
	{
		LteHelper::rsrpVtr.push_back(std::vector<double>());
		LteHelper::rsrpAllVtr.push_back(std::vector<double>());
		LteHelper::sinrVtr.push_back(std::vector<double>());
		LteHelper::tbsVtr.push_back(std::vector<double>());
		for(uint32_t j = 0 ; j < nCells ; j++ )
		{
			LteHelper::rsrpVtr[i].push_back(0.0);
			LteHelper::rsrpAllVtr[i].push_back(0.0);
			LteHelper::sinrVtr[i].push_back(0.0);
			LteHelper::tbsVtr[i].push_back(0.0);
		}
	}
}
...
LteHelper::SetEpcHelper (Ptr<EpcHelper> h)
{
  ...
  if(h==0)
	  std::cout<<"EPC HELPER - OUT of context: "<<std::endl;
  std::cout<<"EPC HELPER - PRINT: "<<h<<std::endl;
  m_epcHelper = h;
}
...
NetDeviceContainer
LteHelper::InstallEnbDevice (NodeContainer c, uint16_t dlearfcn, uint16_t ulearfcn)
{
  NS_LOG_FUNCTION (this);
  ...
  for (NodeContainer::Iterator i = c.Begin (); i != c.End (); ++i)
    {
      Ptr<Node> node = *i;
      Ptr<NetDevice> device = InstallSingleEnbDevice (node, dlearfcn, ulearfcn);
      devices.Add (device);
    }
  return devices;
}
...
NetDeviceContainer
LteHelper::InstallUeDevices (NodeContainer c , uint16_t noOfDevs)
{
	NS_LOG_FUNCTION (this);
	NetDeviceContainer devices;
	...
	for (NodeContainer::Iterator i = c.Begin (); i != c.End (); ++i)
	{
	 ...
	 Ptr<Node> node = *i;
	 std::vector<Ptr<NetDevice> > device = InstallMultipleUeDevice (node , noOfDevs);

	 for(unsigned int j = 0 ; j < noOfDevs ; j++)
	   devices.Add(device.at(j));
...
Ptr<NetDevice>
LteHelper::InstallSingleEnbDevice (Ptr<Node> n , uint16_t dlearfcn , uint16_t ulearfcn)
{
  ...
  //dev->SetAttribute ("CellBias", n->getCellBias());
  dev->SetCellBias(n->getCellBias());

  //ADD
  /*dev->SetDlEarfcn(5180);
  dev->SetUlEarfcn(5730);*/

  dev->SetDlEarfcn (36000);
  dev->SetUlEarfcn (37750);
  ...
  std::cout<<"LTE is gonna call StartCA now "<<std::endl;
  dev->Start ();
...
LteHelper::InstallMultipleUeDevice (Ptr<Node> n , uint16_t noOfDevs)
{
  NS_LOG_FUNCTION (this);
  std::vector<Ptr<NetDevice> > devices;

  std::cout<<"INSTALL MULTIPLE UE DEVICE : noOfDevs = "<<noOfDevs<<std::endl;

  for(uint16_t dev = 0 ; dev < noOfDevs ; dev++ )
  {
  ...
  std::cout<<"While creating RRC : Checkpoint "<<rrc<<std::endl;
  ...
  Ptr<LteUeNetDevice> dev = CreateObject<LteUeNetDevice> (n, phy, mac, rrc, nas, noOfDevs);
  phy->SetDevice (dev);
  dlPhy->SetDevice (dev);
  ulPhy->SetDevice (dev);
  nas->SetDevice (dev);
  ...
  std::cout<<"LTE UE dev calls StartCA"<<std::endl;
  dev->Start ();
  devices.push_back(dev);
  }
  std::cout<<"RETURNING DEVICES - SIZE : "<<devices.size()<<std::endl;
  return devices;
}
...
void
LteHelper::AttachCA (Ptr<NetDevice> ueDevice, Ptr<NetDevice> enbDevice)
{
  NS_LOG_FUNCTION (this);
  ...
  std::cout<<"Attach CA moving on to Connect CA"<<std::endl;
  ueNas->ConnectCA(enbDevice);

  //m_downlinkChannel->AddRx (ueLteDevice->GetPhy ()->GetDownlinkSpectrumPhy ());

  std::cout<<"EPC HELPER - OUT (PRINT): "<<m_epcHelper<<std::endl;
  // tricks needed for the simplified LTE-only simulations
  //if (m_epcHelper == 0)
    //{
	  std::cout<<"UE DEVICE ATTACHMENT TO TARGET ENB"<<std::endl;
...
void
LteHelper::AddDeviceToChannel (Ptr<NetDevice> ueDevice)
{
	Ptr<LteUeNetDevice> ueLteDevice = ueDevice->GetObject<LteUeNetDevice> ();
	m_downlinkChannel->AddRx (ueLteDevice->GetPhy ()->GetDownlinkSpectrumPhy ());
}
...
void
LteHelper::AddeNBToChannel (Ptr<ListPositionAllocator> eNBpositionAllocMacro , uint16_t nUEs)
{
    m_downlinkChannel->AddeNB (eNBpositionAllocMacro , nUEs);
}
...
LteHelper::Attach (Ptr<NetDevice> ueDevice, Ptr<NetDevice> enbDevice)
{
  ...
  if(ueLteDevice->GetTargetEnb())
	  ueLteDevice->GetTargetEnb()->GetRrc()->RemoveUe(ueLteDevice);
...
void
LteHelper::ActivateDedicatedEpsBearerCA (NetDeviceContainer ueDevices, EpsBearer bearer, Ptr<EpcTft> tft)
{
  NS_LOG_FUNCTION (this);
  for (NetDeviceContainer::Iterator i = ueDevices.Begin (); i != ueDevices.End (); ++i)
    {
      ActivateDedicatedEpsBearerCA (*i, bearer, tft);
    }
}


void
LteHelper::ActivateDedicatedEpsBearerCA (Ptr<NetDevice> ueDevice, EpsBearer bearer, Ptr<EpcTft> tft)
{
  NS_LOG_FUNCTION (this);

  NS_ASSERT_MSG (m_epcHelper != 0, "dedicated EPS bearers cannot be set up when EPC is not used");
  
  ueDevice->GetObject<LteUeNetDevice> ()->ActivateDedicatedEpsBearerCA (bearer, tft);
...
LteHelper::ActivateDataRadioBearer (Ptr<NetDevice> ueDevice, EpsBearer bearer)
{
  ...
  std::cout<<"Actviating data radio bearer"<<std::endl;
  //sleep(2);
...
LteHelper::AddX2Interface (NodeContainer enbNodes)
{
  ...
  if(counter == 0)
    fb.open("//home//rajarajan//Documents//att_workspace//ltea//hetnet//x2checks.txt",std::ios::out);
  counter++;
  ...
  for (NodeContainer::Iterator i = enbNodes.Begin (); i != enbNodes.End (); ++i)
    {
      ...
      for (NodeContainer::Iterator j = i + 1; j != enbNodes.End (); ++j)
        {
    	  os<<"Interface between Cell 1 "<<cell1<<" and Cell 2 "<<cell2<<"\n";
    	  std::cout<<"Interface between Cell 1 "<<cell1<<" and Cell 2 "<<cell2<<std::endl;
          AddX2Interface (*i, *j);
          cell2++;
        }
      cell1++;
...
LteHelper::DoHandoverRequest (Ptr<NetDevice> ueDev, Ptr<NetDevice> sourceEnbDev, Ptr<NetDevice> targetEnbDev)
{
  ...
  std::cout<<" DO HANDOVER REQUEST, Time : "<<Simulator::Now().GetMilliSeconds()<<" DO Check RRC, IMSI = "<<imsi<<" Target Cell ID = "<<targetCellId<<std::endl;
...
FindImsiFromEnbRlcPath (std::string path)
{
  ...
  std::cout<<"Printing UE Map Path : "<<ueMapPath<<std::endl;
...
FindImsiFromEnbMac (std::string path, uint16_t rnti)
{
  ...
  std::cout<<"IMSI PASSED ON TO ? "<<imsi<<std::endl;
...
{
	//m_DlSchedulingTrace  mcs
  NS_LOG_FUNCTION (macStats << path);
  static int counter = 0;
  static std::ofstream dlSched;
  if(counter==0)
	  dlSched.open("//home//rajarajan//Documents//att_workspace//ltea//dlSchedtrace.txt");
  counter++;
  uint64_t imsi = 0;
  ...
  if (macStats->ExistsImsiPath (pathAndRnti.str ()) == true)
    {
	  std::cout<<"EXISTS IMSI PATH? - YES"<<std::endl;
      imsi = macStats->GetImsiPath (pathAndRnti.str ());
    }
  else
    {
	  std::cout<<"EXISTS IMSI PATH? - NO"<<std::endl;
  ...
  if (macStats->ExistsCellIdPath (pathAndRnti.str ()) == true)
    {
	  std::cout<<"EXISTS CELL ID PATH? - YES"<<std::endl;
      cellId = macStats->GetCellIdPath (pathAndRnti.str ());
    }
  else
    {
	  std::cout<<"EXISTS CELL ID PATH? - NO"<<std::endl;
      cellId = FindCellIdFromEnbMac (path, rnti);
      macStats->SetCellIdPath (pathAndRnti.str (), cellId);
    }

  dlSched<<"Time : "<<Simulator::Now().GetMilliSeconds()<< " Cell ID : "<<(uint16_t)cellId<<" IMSI : "<<(uint16_t)imsi<<" Frame No. : "<<(uint16_t)frameNo<<" Sub Frame No. "<<(uint16_t)subframeNo<<" RNTI : "<<(uint16_t)rnti<<" MCS TB1 : "<<(uint16_t)mcsTb1<<" Size TB 1 : "<<(uint16_t)sizeTb1<<" MCS TB2 : "<<(uint16_t)mcsTb2<<" Size TB2: "<<(uint16_t)sizeTb2<<"\n";
  macStats->DlScheduling (cellId, imsi, frameNo, subframeNo, rnti, mcsTb1, sizeTb1, mcsTb2, sizeTb2);
}
// m_dlScheduling = TB Size =
void
...
void initiateAllRsrpStats (uint32_t nUEs , uint32_t nCells)
{
	for(uint32_t i = 0 ; i < nUEs ; i++)
	{
		allRsrpVector.push_back(std::vector<double>());
		for(uint32_t j = 0 ; j < nCells ; j++)
			allRsrpVector[i].push_back(0.0);
	}
}

void initiateStats (uint32_t nUEs , uint32_t nCells)
{
	std::cout<<"Initiating RSRP vector with UEs and Cells"<<std::endl;
	for(uint32_t i = 0 ; i < nUEs ; i++)
	{
		rsrpVector.push_back(std::vector<double>());
		sinrVector.push_back(std::vector<double>());
		tbsVector.push_back(std::vector<double>());
		for(uint32_t j = 0 ; j < nCells ; j++)
		{
			rsrpVector[i].push_back(0.0);
			sinrVector[i].push_back(0.0);
			tbsVector[i].push_back(0.0);
		}
	}
}

void printStats (uint32_t nUEs , uint32_t nCells)
{
	//std::cout<<"Printing RSRP vector with UEs and Cells"<<std::endl;
	/*for(uint32_t imsi = 0 ; imsi < nUEs ; imsi++ )
	{
		for(uint32_t cellId = 0 ; cellId < nCells ; cellId++ )
			std::cout<<"rsrpVector["<<imsi<<"]["<<cellId<<"]="<<rsrpVector[imsi][cellId]<<" and in dB, it is "<<10*log(rsrpVector[imsi][cellId])<<std::endl;
	}*/
}

std::vector<double>
LteHelper::receiveRsrp (uint64_t imsi)
{
	return rsrpVtr.at(imsi);
}

std::vector<std::vector<double> >
LteHelper::getRsrpAllVector ()
{
	//std::cout<<"RSRP vector size = "<<allRsrpVector.size()<<" RSRP vector size = "<<rsrpAllVtr.size()<<std::endl;
	//std::cout<<"Just for confirmation : "<<rsrpAllVtr.at(319).at(0)<<" , "<<rsrpAllVtr.at(319).at(1)<<" , "<<rsrpAllVtr.at(319).at(2)<<" , "<<rsrpAllVtr.at(319).at(3)<<std::endl;
//	sleep(2);
	for(uint32_t i = 0 ; i < allRsrpVector.size() ; i++ )
	{
		for(uint32_t j = 0 ; j < allRsrpVector.at(i).size() ; j++ )
			allRsrpVector.at(i).at(j)=0.0;
	}
	return rsrpAllVtr;
}

std::vector<std::vector<double> >
LteHelper::getTbsVector()
{
	return tbsVtr;
}

std::vector<std::vector<double> >
LteHelper::getRsrpVector ()
{
	return rsrpVtr;
}

std::vector<std::vector<double> >
LteHelper::getSinrVector ()
{
	return sinrVtr;
}

void
LteHelper::passRsrpStats(std::vector<std::vector<double> > rsrp)
{
	rsrpAllVtr = rsrp;
	std::cout<<"After passing, size = "<<rsrpAllVtr.size()<<std::endl;
}


void
LteHelper::passRsrpStats(std::vector<std::vector<double> > rsrp , std::vector<std::vector<double> > sinr)
{
	rsrpVtr=rsrp;
	sinrVtr=sinr;
/*	std::cout<<"PASS RSRP STATS : Printing RSRP vector with UEs and Cells"<<std::endl;
		for(uint32_t imsi = 0 ; imsi < rsrpVtr.size() ; imsi++ )
		{
			for(uint32_t cellId = 0 ; cellId < rsrpVtr.at(imsi).size() ; cellId++ )
			{
				if(rsrpVtr.at(imsi).at(cellId)!=0)
				std::cout<<"RSRP STATS : rsrpVector["<<imsi<<"]["<<cellId<<"]="<<rsrpVtr.at(imsi).at(cellId)<<" and in dB, it is "<<10*log(rsrpVtr[imsi][cellId])<<std::endl;
			}
		} */ //m_reportCurrentCellRsrpSinrCallback   rsrp
}

void
LteHelper::passTbsStats (std::vector<std::vector<double> > tbs)
{
	tbsVtr = tbs;
}

void
ReportAllCellRsrpCallback (Ptr<PhyStatsCalculator> phyStats , std::string path, uint16_t cellId, uint64_t imsi, double rsrp)
{
	//std::cout<<"All cell call back"<<std::endl;
	static int counter = 0;
	if(counter==0)
		  initiateAllRsrpStats(noOfUEs,noOfCells);
	counter++;
	allRsrpVector[imsi-1][cellId-1]=rsrp;
   // std::cout<<"Cell ID = "<<cellId<<" IMSI = "<<imsi<<" RSRP = "<<rsrp<<std::endl;
    phyStats->ReportAllCellRsrp(cellId, imsi, rsrp);
    //std::cout<<"RSRP vector size check before passing RSRP vector : "<<allRsrpVector.size()<<std::endl;
    //sleep(2);
	LteHelper::passRsrpStats(allRsrpVector);
}
                      ...
                      double rsrp, double sinr)
{
  ...
  std::cout<<"SHOULDN'T I BE CALLED BACK/NOTIFIED ? "<<std::endl;
  static int counter = 0;
  if(counter==0)
	  initiateStats(noOfUEs,noOfCells);
  NS_LOG_FUNCTION (phyStats << path);
  std::cout<<"PHY STATS PHY STATS PHY STATS"<<phyStats<<std::endl;
  static std::filebuf fb;
  if(counter == 0)
  		 fb.open("//home//rajarajan//Documents//att_workspace//ltea//hetnet//phyStats.txt",std::ios::out);
  static std::ostream os(&fb);
  os<<"PHY STATS PHY STATS PHY STATS"<<phyStats<<" CELL ID: "<<cellId<<" RNTI: "<<rnti<<" RSRP: "<<rsrp<<" SINR: "<<sinr<<"\n";
  uint64_t imsi = 0;
  ...
  std::cout<<"Cell ID : "<<cellId<<" IMSI : "<<imsi<<" RNTI : "<<rnti<<" RSRP : "<<rsrp<<" SINR : "<<sinr<<std::endl;

  rsrpVector[imsi-1][cellId-1]=rsrp;
  sinrVector[imsi-1][cellId-1]=sinr;
  phyStats->ReportCurrentCellRsrpSinr (cellId, imsi, rnti, rsrp,sinr);
  //printStats(5,5);
//  std::cout<<"---------------- CROSS-CHECK ------------------------"<<std::endl;
  LteHelper::passRsrpStats(rsrpVector,sinrVector);
  counter++;
}

void
ReportCurrentCellTbsCallback (Ptr<PhyStatsCalculator> phyStats,
        std::string path, uint16_t cellId , uint16_t rnti , uint16_t tbsSize)
{
	static int counter = 0;
	if(counter==0)
		  initiateStats(noOfUEs,noOfCells);
	uint64_t imsi = 0;
	std::string pathUePhy  = path.substr (0, path.find ("/ReportCurrentCellTbs"));
	if (phyStats->ExistsImsiPath (pathUePhy) == true)
	{
	    imsi = phyStats->GetImsiPath (pathUePhy);
	}
	...
	tbsVector[imsi-1][cellId-1]=tbsSize;
	LteHelper::passTbsStats (tbsVector);
	counter++;
}
...
LteHelper::EnableDlPhyTraces (void)
{
  ...
  Config::Connect ("/ChannelList/*/ReportAllCellRsrp",
                   MakeBoundCallback (&ReportAllCellRsrpCallback, m_phyStats));
  Config::Connect ("/NodeList/*/DeviceList/*/LteUePhy/ReportCurrentCellTbs",
		  	  	   MakeBoundCallback (&ReportCurrentCellTbsCallback, m_phyStats));
  //Config::Connect ("/NodeList/*/DeviceList/*/LteUePhy/LteSpectrumPhy/SingleModelSpectrumChannel/ReportAllCellRsrp",
  //                   MakeBoundCallback (&ReportAllCellRsrpCallback, m_phyStats));*/

  std::cout<<"Enable PHY traces"<<std::endl;
}
              ...
              uint16_t cellId, uint16_t rnti, double sinrLinear)
{
  ...
  if (phyStats->ExistsImsiPath (pathAndRnti.str ()) == true)
    {
	  std::cout<<"REPORTING UE SINR : PATH FOUND ? - TRUE"<<std::endl;
      imsi = phyStats->GetImsiPath (pathAndRnti.str ());
    }
  else
    {
	  std::cout<<"REPORTING UE SINR : PATH FOUND ? - FALSE"<<std::endl;
