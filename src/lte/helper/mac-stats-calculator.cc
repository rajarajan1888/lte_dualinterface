                                  ...
                                  uint16_t rnti, uint8_t mcsTb1, uint16_t sizeTb1, uint8_t mcsTb2, uint16_t sizeTb2)
{
  ...
  std::cout<<"Write DL Mac Stats in " << GetDlOutputFilename ().c_str ()<<std::endl;
  ...
  if ( m_dlFirstWrite == true )
    {
      ...
      if (!outFile.is_open ())
        {
          //std::cout<<"Issue in opening file"<<std::endl;
          std::cout<<"cannot open file here: I'm sorry here OH OH OH - MacStats"<<std::endl;
  ...
  else
    {
      ...
      if (!outFile.is_open ())
        {
          //std::cout<<"Issue in opening file - Case 2"<<std::endl;
    	  std::cout<<"cannot open file here: I'm sorry here OH OH OH - MacStats App"<<std::endl;
          NS_LOG_ERROR ("Can't open file " << GetDlOutputFilename ().c_str ());
          return;
        }
      std::cout<<"Write DL Mac Stats in : APP " << GetDlOutputFilename ().c_str ()<<std::endl;
    }
  ...
  std::cout<<"Cell ID: "<<cellId<<" ,IMSI: "<<imsi<<", Frame No: "<<frameNo<<" , Sub Frame No: "<<subframeNo<<" ,MCS TB1: "<<mcsTb1<<" ,MCS TB2: "<<mcsTb2<<std::endl;
  outFile.close ();
}
