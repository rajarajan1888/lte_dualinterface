...
// RAJARAJAN
void
EpcHelper::AddEnbCA (Ptr<Node> enb, Ptr<NetDevice> lteEnbNetDevice, uint8_t maxCCs)
{
  ...
  static NodeContainer enbSgwNodes;
  static PointToPointHelper p2ph;
  static NetDeviceContainer enbSgwDevices;
  static Ipv4Address enbAddress, sgwAddress;
  static Ipv4InterfaceContainer enbSgwIpIfaces;
  static int retval;
  static Ptr<Socket> enbS1uSocket;
  ...
  // the corresponding new NetDevices on each side
          ...
          enbSgwDevices = p2ph.Install (enb, m_sgwPgw);
          ...
          enbSgwIpIfaces = m_s1uIpv4AddressHelper.Assign (enbSgwDevices);
  ...
  NS_ASSERT (enb->GetNApplications () <= maxCCs);
  NS_ASSERT_MSG (enb->GetApplication (index % maxCCs)->GetObject<EpcEnbApplication> () != 0, "cannot retrieve EpcEnbApplication");
  NS_LOG_LOGIC ("enb: " << enb << ", enb->GetApplication ("<<(index % maxCCs)<<"): " << enb->GetApplication (index%maxCCs));
...
EpcHelper::AddX2Interface (Ptr<Node> enb1, Ptr<Node> enb2)
{
  ...
  // the corresponding new NetDevices on each side  SendPacket ->Send (
...
void
EpcHelper::AttachUeCA (Ptr<NetDevice> ueLteDevice, uint64_t imsi, Ptr<NetDevice> enbDevice)
{
  //std::multimap<uint64_t, Ptr<NetDevice> > m_imsiEnbDeviceMultiMap;
  NS_LOG_FUNCTION (this << ueLteDevice << enbDevice);
  std::cout<<"IMSI attach = "<<imsi<<std::endl;
  m_imsiEnbDeviceMultiMap.insert(std::pair<uint64_t,Ptr<NetDevice> >(imsi, enbDevice));

}


void
EpcHelper::ActivateEpsBearerCA (Ptr<NetDevice> ueLteDevice, uint64_t imsi, Ptr<EpcTft> tft, EpsBearer bearer)
{
	static int index = 0;
	std::pair<std::multimap<uint64_t, Ptr<NetDevice> >::iterator , std::multimap<uint64_t , Ptr<NetDevice> >::iterator > m_imsiEnbMap;
	std::cout<<"IMSI ("<<imsi<<") - eNB Device Map count (check) = "<<m_imsiEnbDeviceMultiMap.count(imsi)<<std::endl;
	NS_ASSERT_MSG(m_imsiEnbDeviceMultiMap.count(imsi) != 0, "no eNB found for this IMSI, did you attach it before?");
	m_imsiEnbMap = m_imsiEnbDeviceMultiMap.equal_range(imsi);
	std::multimap<uint64_t , Ptr<NetDevice> >::iterator it = m_imsiEnbMap.first;
	std::cout<<"Index % maxCCs = "<<(index%maxCCs)<<std::endl;
	...
	int32_t ueInterface =  ueIpv4->GetInterfaceForDevice (ueLteDevice);
	std::cout<<"Number of addresses = "<<ueIpv4->GetNAddresses (ueInterface)<<std::endl;
	NS_ASSERT (ueInterface >= 0);
	NS_ASSERT (ueIpv4->GetNAddresses (ueInterface) <= 1);
	Ipv4Address ueAddr = ueIpv4->GetAddress (ueInterface, 0).GetLocal ();
	...
	//NS_LOG_LOGIC ("enb: " << enbNode << ", enb->GetApplication ("<<(index%maxCCs)<<"): " << enbNode->GetApplication (0));
	NS_LOG_LOGIC ("enb: " << enbNode << ", enb->GetApplication ("<<(index%maxCCs)<<"): " << enbNode->GetApplication (0));
	NS_ASSERT (enbNode->GetNApplications () <= maxCCs);
	...
	std::cout<<"Problem is from here?"<<std::endl;
	epcEnbApp->ErabSetupRequestCA (teid, imsi, bearer, maxCCs);
	index++;
}

void
EpcHelper::ReactivateEpsBearer (Ptr<NetDevice> ueLteDevice, uint64_t imsi)
{
	  ...
	  NS_LOG_LOGIC ("number of Ipv4 ifaces of the eNB: " << enbIpv4->GetNInterfaces ());
	  // two ifaces total: loopback + the S1-U interface
	  std::cout<<"Number of interfaces for eNB IPv4 : "<<enbIpv4->GetNInterfaces()<<std::endl;
	  //NS_ASSERT (enbIpv4->GetNInterfaces () == 4);
	  //WILD HACK -- RAJARAJAN
	  NS_ASSERT (enbIpv4->GetNInterfaces () == 2);
	  //WILD HACK -- RAJARAJAN --
	  NS_ASSERT (ueIpv4->GetNAddresses (1) == 1);
	  ...
	  Ipv4Address enbAddr = enbIpv4->GetAddress (1, 0).GetLocal ();
	  NS_LOG_LOGIC (" ENB IP address: " << enbAddr);
	  m_sgwPgwApp->ReactivateS1Bearer (ueAddr, enbAddr);
	  ...
	  Ptr<EpcEnbApplication> epcEnbApp = app->GetObject<EpcEnbApplication> ();
	  NS_ASSERT (epcEnbApp != 0);
	  std::map <uint32_t, Ptr<EpcTft> > m_classifierEpcTft = m_sgwPgwApp->GetClassifierEpcTft(ueAddr);
	  std::vector<EpsBearer> bearerVtr = m_sgwPgwApp->GetBearers(imsi);
	  uint8_t bearerIndex = 0;
	  std::cout<<" Epc TFT classifier size = "<<m_classifierEpcTft.size()<<" and bearer Vector size = "<<bearerVtr.size()<<std::endl;
	  for(std::map <uint32_t, Ptr<EpcTft> >::iterator it = m_classifierEpcTft.begin() ; it!=m_classifierEpcTft.end(); it++)
	  {
		  epcEnbApp->ErabSetupRequest(it->first, imsi, bearerVtr.at(bearerIndex) );
		  bearerIndex++;
	  }
}
//AddApplication
...
{
  std::cout<<"Activate EPS Bearer"<<std::endl;
  std::map<uint64_t, Ptr<NetDevice> >::iterator it = m_imsiEnbDeviceMap.find (imsi);
  NS_ASSERT_MSG (it != m_imsiEnbDeviceMap.end (), "no eNB found for this IMSI, did you attach it before?");
  Ptr<NetDevice> enbDevice = it->second;
  Ptr<Node> ueNode = ueLteDevice->GetNode ();
  ...
  // setup S1 bearer at EpcSgwPgwApplication
  uint32_t teid = m_sgwPgwApp->ActivateS1Bearer (ueAddr, enbAddr, tft);
  m_sgwPgwApp->MaintainBearerMap(imsi , bearer);
