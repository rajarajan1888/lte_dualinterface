...
int main (int argc, char *argv[])
{
        ...
        for (uint32_t j = 0; j < nUe_cre; j++)
          {
            if ( i == nEnb - 3 )
              ...
              {
            		//positionAlloc->Add (Vector (posX->GetValue (), posY->GetValue (), nodeHeight));
                positionAlloc->Add (Vector (enbPosition.at(i).x + (rand() % (int)(roomLength * 163 * metroUeXmin)),
  										  										enbPosition.at(i).y + (rand() % (int)(roomLength * 163 * metroUeXmin)),
  										  										nodeHeight));
              }
            mobility.SetPositionAllocator (positionAlloc);
          }
  ...
  vector <NetDeviceContainer> uDevsf1;
  vector <NetDeviceContainer> uDevsf2;
  vector <NetDeviceContainer> uDevsCreMetro;
  vector <NetDeviceContainer> uDevsCreMacro;
  vector <NetDeviceContainer> uDevsXtra;
  ...
  // For each of the following cases, scenario-specific topologies need to be created.
  	...
  	for (uint32_t i = 0; i < nEnb; i++)
    {
      ...
      ueDevs_f2.Add (ueDev);
      uDevsf2.push_back(ueDev);
      //lteHelper->Attach (ueDev, enbDevs_f2.Get (i));
    }
	}
	if (eicic && !icic) { // eICIC topological setup
		// Assign only a subset of the metro CRE UEs (e.g. only one) to the metro-cre eNB,
    ...
    uDevsCreMetro.push_back(ueDev_metro_cre);
    ...
    for (uint32_t j = 1; j < nUe_cre; j++)
    {
    	...
    	uDevsCreMetro.push_back(ueDev_metrocre);
    }

    // Assign all the macro CRE UEs to the macro eNB:
    NetDeviceContainer ueDevDummy;
    uDevsCreMacro.push_back(ueDevDummy);
    ...
    for (uint32_t j = 0; j < nUe_xtra; j++)
    {
      ...
      uDevsXtra.push_back(ueDev_metrox);
    }
	}
  ...
  if (epc)
    {
  	...
  	// Device attachments: Need to be performed irrespective of epc==1, and
  	// after IP assignment (for ns3 versions after 3.15):
     for (uint32_t i = 0; i < nEnb; i++)
     {
     	lteHelper->Attach (uDevsf1.at(i), enbDevs.Get (i));
     }

    if (icic && !eicic) { // ICIC attachments
     	for (uint32_t i = 0; i < nEnb; i++)
       {
         lteHelper->Attach (uDevsf2.at(i), enbDevs_f2.Get (i));
       }
    }
    if (eicic && !icic) { // eICIC attachments - edw
    	// Attach the metro CRE UE to the "metro CRE" eNB
      lteHelper->Attach (uDevsCreMetro.at(0), enbDevs_cre.Get (0));
     	//lteHelper->Attach (uDevsCreMetro.at(0), enbDevs.Get (0));
     	//lteHelper->Attach (uDevsCreMetro.at(0), enbDevs.Get (3));

      for (uint32_t j = 1; j < nUe_cre; j++)
       {
       	// Attach the remaining metro CRE UEs to metro default:
       	lteHelper->Attach (uDevsCreMetro.at(j), enbDevs.Get (0));
       }

       // Assign all the macro CRE UEs to the macro eNB:
    	for (uint32_t i = 1; i < nEnb; i++)
       {
         lteHelper->Attach (uDevsCreMacro.at(i), enbDevs.Get (i));
       }

       // Assign extra macro CRE UEs to the macro eNB:
    	for (uint32_t j = 0; j < nUe_xtra; j++)
       {
       	// Attach the xtra UEs to macro:
       	lteHelper->Attach (uDevsXtra.at(j), enbDevs.Get (3));
       }
    }
      ...
      for (uint32_t i = 0; i < nEnb; i++)
       {
         ...
         lteHelper->Attach (uDevsf2.at(i), enbDevs.Get (i));
       }
    }
