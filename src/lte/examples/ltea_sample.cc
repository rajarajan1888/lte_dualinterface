...
main (int argc, char *argv[])
{
  ...
  Ptr<LteAdvancedHelper> lteAdvancedHelper = CreateObject<LteAdvancedHelper> ();
  ...
  for(unsigned int ueId = 0 ; ueId < ueNodes.GetN() ; ueId++)
  {
	  //std::cout<<"Pushing value : "<<epcHelper->AssignUeIpv4Address(ueLteDevs.at(ueId))<<std::endl;
  	  ueIpIface.push_back(epcHelper->AssignUeIpv4Address(ueLteDevs.at(ueId)));
  }

  std::cout<<"Size of UE IP Iface vector = "<<ueIpIface.size()<<std::endl;

  /*for(unsigned int dev = 0 ; dev < ueLteDevs.size() ; dev++)
  ueIpIface.push_back(epcHelper->AssignUeIpv4Address (ueLteDevs.at(dev)));*/
  // Assign IP address to UEs, and install applications
  	for (uint32_t u = 0; u < ueLteDevs.size(); ++u)
    {
  	  for(NetDeviceContainer::Iterator it = ueLteDevs.at(u).Begin();it!=ueLteDevs.at(u).End();it++)
  	  {
  		 Ptr<Node> ueNode = (*it)->GetNode();
  		 Ptr<Ipv4StaticRouting> ueStaticRouting = ipv4RoutingHelper.GetStaticRouting (ueNode->GetObject<Ipv4> ());
  		 ueStaticRouting->SetDefaultRoute (epcHelper->GetUeDefaultGatewayAddress (), 1);
  	  }

      //Ptr<NetDevice> ueNode = ueLte.Get (u);
  ...
  // Attach one UE per eNodeB
    lteAdvancedHelper->AttachCA(ueLteDevs,enbLteDevs.at(0));
...
//  PacketSinkHelper packetSinkHelper ("ns3::UdpSocketFactory", InetSocketAddress (Ipv4Address::GetAny (), otherPort));
  ...
  //    serverApps.Add (packetSinkHelper.Install (ueNodes.Get(u)));
      ...
      unsigned int devctr = 0;
      for(NetDeviceContainer::Iterator dev = ueLteDevs.at(u).Begin() ; dev != ueLteDevs.at(u).End() ; dev++)
      {
    	//  std::cout<<"Total number of addresses : "<<ueIpIface.at(u).GetN()<<std::endl;
    	  UdpClientHelper dlClient (ueIpIface.at(u).GetAddress(devctr,0), dlPort);
    	  devctr++;
...
/*      if (u+1 < ueNodes.GetN ())
  ...
  lteAdvancedHelper->EnableTracesAdvanced ();
  lteAdvancedHelper->EnableMacTracesAdvanced ();
  lteAdvancedHelper->EnableRlcTracesAdvanced ();
  ...
  /*GtkConfigStore config;  ConfigureCell
