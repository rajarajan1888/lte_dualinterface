...
{
  NS_LOG_FUNCTION_NOARGS ();
 // std::cout<<this<<" SOCKET SEND: Time = "<<Simulator::Now().GetMilliSeconds()<<std::endl;
   // 	    sleep(5);
  return Send (p, 0);
}
...
{
  NS_LOG_FUNCTION_NOARGS ();
 // std::cout<<"PACKET SEND TO WITH SIZE = "<<size<<std::endl;
 // sleep(4);
  Ptr<Packet> p;
  if(buf)
    {
...
Socket::NotifyConnectionSucceeded (void)
{
  ...
  if (!m_connectionSucceeded.IsNull ())
    {
	//  std::cout<<this<<" Notify Connection Succeeded: Time = "<<Simulator::Now().GetMilliSeconds()<<std::endl;
	// 	        	    sleep(5);
...
{
  NS_LOG_FUNCTION_NOARGS ();
  std::cout<<"Data sent notification - Packet size : "<<size<<std::endl;
  if (!m_dataSent.IsNull ())
    {
	  std::cout<<"NOT NULL- Packet size : "<<size<<std::endl;
