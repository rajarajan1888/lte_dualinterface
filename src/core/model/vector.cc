...
CalculateDistance (const Vector3D &a, const Vector3D &b)
{
  ...
  static std::ofstream myDist;
  static int counter = 0;
  if(counter==0)
	  myDist.open("//home//rajarajan//Documents//att_workspace//ltea//distcomp.txt");
  counter++;
  double dx = b.x - a.x;
  double dy = b.y - a.y;
  double dz = b.z - a.z;
  double distance = std::sqrt (dx * dx + dy * dy + dz * dz);
  	  myDist<<"(X1,Y1,Z1) = ("<<b.x<<","<<b.y<<","<<b.z<<") and (X2,Y2,Z2) = ("<<a.x<<","<<a.y<<","<<a.z<<") and Distance = "<<distance<<"\n";
  return distance;
}
double 
