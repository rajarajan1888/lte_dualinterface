...
Ipv4Interface::Send (Ptr<Packet> p, Ipv4Address dest)
{
  ...
  std::cout<<"Sent to IPv4 interface : "<<std::endl;
      ...
      if (dest == (*i).GetLocal ())
        {
          ...
          std::cout<<" IPv4 L3 protocol SEND"<<std::endl;
          sleep(3);
  ...
  if (m_device->NeedsArp ())
      ...
      if (found)
        {
          ...
          std::cout<<"ARP resolved?"<<std::endl;
          	  sleep(2);
  ...
  else
    {
      ...
      std::cout<<"ARP not required? :)"<<std::endl;
    //  	  sleep(2);
    //  m_device = DynamicCast<LoopbackNetDevice> (m_device);
    /*  Ptr<LoopbackNetDevice> m_loopbackDevice = DynamicCast<LoopbackNetDevice> (m_device);
      m_loopbackDevice->Send(p,m_loopbackDevice->GetBroadcast(),Ipv4L3Protocol::PROT_NUMBER); */
    int x =  m_device->Send (p, m_device->GetBroadcast (),
                      Ipv4L3Protocol::PROT_NUMBER); // : public NetDevice
    std::cout<<" Sent from device with value "<<x<<std::endl;
    //sleep(2);
    }
}
