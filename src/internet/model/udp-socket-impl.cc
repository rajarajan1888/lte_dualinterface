...
UdpSocketImpl::DoSendTo (Ptr<Packet> p, Ipv4Address dest, uint16_t port)
{
  ...
  if (dest.IsBroadcast ())
          ...
          if (maski == Ipv4Mask::GetOnes ())
            {
              ...
              std::cout<<"IS BROADCAST FOR LOOP : "<<std::endl;
              //sleep(2); Sent from device with value
          ...
          else
            {
              ...
              m_udp->Send (p->Copy (), addri, bcast,
                           m_endPoint->GetLocalPort (), port);
              std::cout<<"ELSE FOR LOOP : "<<std::endl;
                //            sleep(2);
              NotifyDataSent (p->GetSize ());
              NotifySend (GetTxAvailable ());
            }
  ...
  else if (m_endPoint->GetLocalAddress () != Ipv4Address::GetAny ())
    {
      ...
      std::cout<<"local address - end point : "<<std::endl;
      ...
      if (route != 0)
        {
          ...
          std::cout<<"rOUTING PROTOCOL FOR LOOP : "<<std::endl;
                  //      sleep(2);
...
{
	std::cout<<"udp socket impl : PACKET SEND TO WITH SIZE WITH FLAGS "<<flags<<std::endl;
	 //   sleep(2);
