  ...
  if (m_state == ESTABLISHED || m_state == SYN_SENT || m_state == CLOSE_WAIT)
    {
      ...
      if (m_state == ESTABLISHED || m_state == CLOSE_WAIT)
        { // Try to send the data out
    	  std::cout<<" TCP Socket Base Send Data Packet transmission "<<std::endl;
    	      //sleep(5); //SEND Data: Time
...
{
	std::cout<<this<<" BEFORE SENDING TO : Time = "<<Simulator::Now().GetMilliSeconds()<<std::endl;
	    sleep(5);
...
TcpSocketBase::SetupCallback (void)
{
  ...
  if (m_endPoint != 0)
    {
	  /*std::cout<<"Starting OnOffApplication TCP Socket Base - START"<<std::endl;
	  sleep(10);*/
...
{
  /*std::cout<<"OnOffApplication TCP Socket Base - Forward Up"<<std::endl;
  sleep(10);*/
                            ...
                            Ptr<Ipv4Interface> incomingInterface)
{
  ...
  /*std::cout<<"TCP Socket Base "<< tcpHeader.GetAckNumber() <<"- Do Forward Up "<<(tcpHeader.GetFlags()&TcpHeader::ACK)<<std::endl;
  sleep(10);*/

  uint8_t tcpflags = tcpHeader.GetFlags () & ~(TcpHeader::PSH | TcpHeader::URG);

  std::cout<<" TCP Header flag = "<<(uint16_t)tcpflags<<std::endl;
  sleep(3);

  if (tcpHeader.GetFlags () & TcpHeader::ACK)
    {
	  /*std::cout<<"TCP Header - ACK"<<std::endl;
	  sleep(10);*/
  ...
  std::cout<<" TCP Header Window size = "<<m_rWnd<<std::endl;
  //sleep(3);
...
TcpSocketBase::SendDataPacket (SequenceNumber32 seq, uint32_t maxSize, bool withAck)
{
  ...
  std::cout<<" TCP Header ACK - Send Data Packet with size "<<sz<<std::endl;
    sleep(5);
  ...
  while (m_txBuffer.SizeFromSequence (m_nextTxSequence))
    {
      ...
      std::cout<<" TCP Header ACK - Original Data Packet transmission"<<std::endl;
      sleep(5);
...
TcpSocketBase::EstimateRtt (const TcpHeader& tcpHeader)
{
  ...
  // is implemented, this function would be more elaborated.
	static std::ofstream rtt;
	static int counter = 0;
	if(counter==0)
		rtt.open("//home//rajarajan//Documents//att_workspace//ltea//hetnet//lastRtt.txt");
	counter++;
  m_lastRtt = m_rtt->AckSeq (tcpHeader.GetAckNumber () );
  rtt<<this<<" Time : "<<Simulator::Now().GetMilliSeconds()<< "Source : "<<tcpHeader.GetSourcePort()<<" Destination : "<<tcpHeader.GetDestinationPort()<<" RTT : "<<m_lastRtt<<"\n";
  std::cout<<this<<" Time : "<<Simulator::Now().GetMilliSeconds()<< "Source : "<<tcpHeader.GetSourcePort()<<" Destination : "<<tcpHeader.GetDestinationPort()<<" RTT : "<<m_lastRtt<<std::endl;
  sleep (5);
}
