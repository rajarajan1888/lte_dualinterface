...
void BulkSendApplication::StartApplication (void) // Called at time specified by Start
{
  ...
  if (m_connected)
    {
	  std::cout<<this<<" Data SEND Initiation: Time = "<<Simulator::Now().GetMilliSeconds()<<std::endl;
	        	    sleep(5);
      SendData ();
    }
}
...
void BulkSendApplication::SendData (void)
{
  ...
  while (m_maxBytes == 0 || m_totBytes < m_maxBytes)
    { // Time to send more
      ...
      m_txTrace (packet);
      std::cout<<this<<" Bulk Application SEND Data: Time = "<<Simulator::Now().GetMilliSeconds()<<std::endl;
      //sleep(5);
...
void BulkSendApplication::ConnectionSucceeded (Ptr<Socket> socket)
{
  ...
  m_connected = true;
  std::cout<<this<<" Connection for Data send: Time = "<<Simulator::Now().GetMilliSeconds()<<std::endl;
        	    sleep(5);
  SendData ();
}
